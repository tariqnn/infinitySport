...
function is3x3(row: CompetitionRegistrationRow) {
  return (
    row.competitionType === "3X3" ||
    row.competitionType === "3X3_MEN" ||
    row.competitionType === "3X3_WOMEN"
  );
}

function isActive3x3(row: CompetitionRegistrationRow) {
  return (
    row.competitionType === "3X3_MEN" || row.competitionType === "3X3_WOMEN"
  );
}
...
function defaultCompetitionRate(type: string) {
  return isTeamCompetition(type) ? 50 : 25;
}

function competitionRate(row: CompetitionRegistrationRow) {
  return row.amountDue ?? defaultCompetitionRate(row.competitionType);
}
...
function registrationRows(rows: CompetitionRegistrationRow[], filter: string) {
  const filteredRows =
    filter === "ALL"
      ? rows
      : rows.filter((row) => row.competitionType === filter);
...
function exportRegistrationsCsv(
  ...
  downloadCsv(`${title}-registrations.csv`, [
    registrationHeaders(),
    ...registrationRows(rows, filter),
  ]);
}
...
function exportRegistrationsPdf(
  ...
  const title =
    filter === "ALL"
      ...
      : `${competitionLabel(filter)} registrations`;
      ...
      group.games.length > 0
        ...
        : [
            [
              ...
              group.groupName,
              "",
              teamLabel(group.teams[0]),
              "No games",
              competitorNames(group.teams[0]),
              "",
            ],
          ],
    ),
...
function build3x3Groups(
  ...
  const teams = rows.filter(
    (row) =>
      isActive3x3(row) && (filter === "ALL" || row.competitionType === filter),
  );
  ...
  for (const [division, divisionTeams] of byDivision.entries()) {
    ...
    for (
      let groupIndex = 0;
      groupIndex < divisionGroups.length;
      groupIndex += 1
    ) {
...
function EditCompetitionModal({
  ...
  const suggestedRate = defaultCompetitionRate(competitionType);
  const competitionOptions = COMPETITION_TYPES.some(
    (option) => option.value === currentRow.competitionType,
  )
    ? COMPETITION_TYPES
    : [
        {
          value: currentRow.competitionType,
          label: competitionLabel(currentRow.competitionType),
        },
        ...COMPETITION_TYPES,
      ];
    ...
    try {
      const updated = await competitionRegistrationsApi.update(currentRow.id, {
        ...
        amountDue: amountDue.trim()
          ? Math.max(0, Math.round(Number(amountDue) || 0))
          : null,
        amountPaid: amountPaid.trim()
          ? Math.max(0, Math.round(Number(amountPaid) || 0))
          : null,
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to save competition registration.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Modal
      open={!!row}
      onClose={onClose}
      title="Edit competition registration"
      size="lg"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-4">
          <Select
            label="Paid"
            value={isPaid ? "yes" : "no"}
            onChange={(event) => setIsPaid(event.target.value === "yes")}
          >
            <option value="no">Unpaid</option>
            <option value="yes">Paid</option>
          </Select>
          <Input
            label={`Prize rate (${suggestedRate} JOD default)`}
            type="number"
            min={0}
            value={amountDue}
...
export default function CompetitionsPage() {
  ...
  const [editingRow, setEditingRow] =
    useState<CompetitionRegistrationRow | null>(null);
  ...
  const totals = useMemo(() => {
    ...
    for (const row of filteredRows) {
      byCompetition.set(
        row.competitionType,
        (byCompetition.get(row.competitionType) || 0) + 1,
      );
    }
  ...
  const threeXThreeCount = useMemo(
    () =>
      rows.filter(
        (row) =>
          isActive3x3(row) &&
          (filter === "ALL" || row.competitionType === filter),
      ).length,
    [filter, rows],
  );
  ...
  async function togglePaid(
    row: CompetitionRegistrationRow,
    nextPaid: boolean,
  ) {
    setSavingPaymentId(row.id);
    try {
      const amountText = nextPaid
        ? window.prompt(
            "Amount paid (JOD)",
            row.amountPaid != null ? String(row.amountPaid) : "",
          )
        : null;
      if (nextPaid && amountText === null) return;
      const amountPaid =
        nextPaid && amountText?.trim()
          ? Math.max(0, Math.round(Number(amountText) || 0))
          : null;
  ...
  async function deleteRegistration(row: CompetitionRegistrationRow) {
    ...
    try {
      ...
      setRows(nextRows);
      setGroupSchedules((prev) =>
        prev.length > 0 ? build3x3Groups(nextRows, filter, groupSize) : prev,
      );
    } catch (err) {
      alert(
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-ui-textMuted">
            Events
          </p>
          <h1 className="mt-1 text-3xl font-bold text-ui-textPrimary">
            Competitions
          </h1>
          <p className="mt-2 text-sm text-ui-textMuted">
            Website registrations for 3x3, King / Queen, Jack of the Court, 3
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
        <Card>
          <CardBody>
            ...
            <p className="mt-2 text-3xl font-bold text-ui-textPrimary">
              {filteredRows.length}
            </p>
          </CardBody>
        </Card>
        <Card>
          <CardBody>
            ...
            <p className="mt-2 text-3xl font-bold text-ui-textPrimary">
              {formatMoney(paymentSummary.collected)}
            </p>
          </CardBody>
        </Card>
        <Card>
          <CardBody>
            ...
            <p className="mt-2 text-3xl font-bold text-ui-textPrimary">
              {formatMoney(paymentSummary.amountDue)}
            </p>
          </CardBody>
        </Card>
        <Card>
          <CardBody>
            ...
            {paymentSummary.unpaidWithUnknownFee > 0 ? (
              <p className="mt-1 text-xs text-ui-textMuted">
                {paymentSummary.unpaidWithUnknownFee} unpaid without amount due
              </p>
            ) : null}
          </CardBody>
        </Card>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {totals.slice(0, 4).map(([type, count]) => (
          <Card key={type}>
            <CardBody>
              <p className="truncate text-xs font-semibold uppercase tracking-[0.16em] text-ui-textMuted">
        ...
        <Card>
          <CardHeader>
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
              <div>
                <h2 className="text-xl font-semibold text-ui-textPrimary">
                  3x3 shuffled groups
                </h2>
                <p className="mt-1 text-sm text-ui-textMuted">
                  Teams are randomized into groups, then every team plays the
                  other teams in its group.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  type="button"
                  variant="secondary"
                  onClick={() => exportGroupsPdf(groupSchedules)}
                >
                  Export PDF
                </Button>
                <Button
          ...
          <CardBody>
            <div className="grid gap-3 md:grid-cols-2">
              {groupSchedules.map((group) => (
                <div
                  key={`${group.division}-${group.groupName}`}
                  className="rounded-lg border border-ui-border bg-ui-softBg/50 p-4"
                >
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-ui-textMuted">
                    {competitionLabel(group.division)} - {group.groupName}
                  </p>
                  <p className="mt-2 text-sm font-semibold text-ui-textPrimary">
                    {group.teams.map(teamLabel).join(" / ")}
                  </p>
                  <div className="mt-3 space-y-2">
                    {group.games.length > 0 ? (
                      group.games.map((game) => (
                        <div
                          key={game.gameNumber}
                          className="rounded-md bg-white px-3 py-2 text-sm text-ui-textPrimary"
                        >
                          Game {game.gameNumber}: {teamLabel(game.teamA)} vs{" "}
                          {teamLabel(game.teamB)}
                        </div>
                      ))
                    ) : (
                      <div className="rounded-md bg-white px-3 py-2 text-sm text-ui-textMuted">
            ...
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:items-end">
              ...
              <div className="flex flex-wrap justify-end gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  variant="secondary"
                  onClick={() => handleRegistrationExport("csv")}
                  disabled={rows.length === 0}
                >
                  Export CSV
                </Button>
                <Button
                  ...
                  size="sm"
                  variant="secondary"
                  onClick={() => handleRegistrationExport("pdf")}
                  disabled={rows.length === 0}
                >
                  Export PDF
                </Button>
              ...
              <table className="min-w-full divide-y divide-ui-border text-sm">
                <thead>
                  <tr className="text-left">
                    <th className="px-4 py-3 font-semibold text-ui-textPrimary">
                      Competition
                    </th>
                    <th className="px-4 py-3 font-semibold text-ui-textPrimary">
                      Name / Team
                    ...
                    <th className="sticky right-0 z-30 w-[72px] min-w-[72px] border-l border-ui-border bg-slate-50 px-2 py-3 text-center font-semibold text-ui-textPrimary shadow-[-6px_0_10px_rgba(15,23,42,0.06)]">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-ui-border">
                  {loading && rows.length === 0 ? (
                    <tr>
                      <td
                        className="px-4 py-8 text-center text-ui-textMuted"
                        colSpan={11}
                      >
                    ...
                    filteredRows.map((row) => (
                      <tr key={row.id} className="hover:bg-ui-softBg/70">
                        <td className="px-4 py-3 font-medium text-ui-textPrimary">
                          {competitionLabel(row.competitionType)}
                        </td>
                        <td className="px-4 py-3 text-ui-textPrimary">
                          {row.teamName || row.participantName || "-"}
                        </td>
                        ...
                        <td className="min-w-[260px] px-4 py-3 text-ui-textMuted">
                          {competitorNames(row)}
                        </td>
                        <td className="px-4 py-3 font-semibold text-ui-textPrimary">
                          {formatMoney(competitionRate(row))}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex flex-col gap-1">
                            <Badge variant={row.isPaid ? "success" : "danger"}>
                              {row.isPaid ? "Paid" : "Unpaid"}
                            </Badge>
                            <span className="text-xs text-ui-textMuted">
                              Paid: {row.amountPaid ?? 0} JOD / Due:{" "}
                              {competitionRate(row)} JOD
                            </span>
                            {row.paymentMethod ? (
                              <span className="text-xs text-ui-textMuted">
                                {row.paymentMethod}
