...
function computeFinalPriceJod(basePriceJod: number, discountType: string, discountValue: number | null | undefined) {
...
function billingPeriodFromDate(date: Date): { billingPeriodKey: string; priceLockedUntil: Date } {
...
async function createPackageRegistration(payload: RegistrationInput) {
  ...
  const basePriceJod = clampNonNegative(
    payload.basePriceJod != null ? Number(payload.basePriceJod) : await getBasePriceJod(packageName),
  );

  const discountType = (payload.discountType || 'NONE').toUpperCase();
  const discountValue = discountType === 'NONE' ? null : Number(payload.discountValue ?? 0);
  ...
  const now = new Date();
  const periodStartsAt = payload.periodStartsAt ? new Date(payload.periodStartsAt) : null;
...
async function listPackageRegistrations(request: NextRequest) {
  const packageName = request.nextUrl.searchParams.get('packageName') || undefined;
...
async function getRegistrationTotals(request: NextRequest) {
  ...
  for (const reg of regs) {
    ...
    const collected = (reg.receipts || []).reduce((sum, rec) => sum + (rec.amountPaid || 0), 0);
...
async function getDashboardStats(request: NextRequest) {
  ...
  const [totalMembers, activeCoaches, activeClasses, activeSubscriptions, pendingBookings, pendingInvoices, openTasks, lowInventory] =
    await Promise.all([
      prisma.member.count({ where: companyWhere }),
      prisma.coach.count({ where: { ...companyWhere, status: 'ACTIVE' as any } }),
      prisma.class.count({ where: { ...companyWhere, status: 'SCHEDULED' as any } }),
      prisma.subscription.count({ where: { ...companyWhere, status: 'ACTIVE' as any } }),
      prisma.booking.count({ where: { ...companyWhere, status: 'PENDING' as any } }),
      prisma.invoice.count({ where: { ...companyWhere, status: { in: ['DRAFT', 'SENT', 'OVERDUE'] as any[] } } }),
      prisma.staffTask.count({ where: { ...companyWhere, status: { in: ['OPEN', 'IN_PROGRESS'] as any[] } } }),
      prisma.inventoryItem.count({ where: { ...companyWhere, status: { in: ['LOW', 'OUT_OF_STOCK'] as any[] } } }),
    ]);
...
async function bulkCreatePackageRegistrations(request: NextRequest) {
  ...
  const results: Array<{ success: boolean; id?: string; row?: number; error?: string }> = [];
...
async function bulkCreateForPerson(request: NextRequest) {
  ...
  if (!customerName || !customerPhone) return jsonError('Person name and phone are required');
    ...
    for (const entry of body.registrations) {
      ...
      const row = await tx.packageRegistration.create({
        data: {
          ...
          discountType,
          discountValue: discountType === 'NONE' ? null : Number(discountValue),
          discountReason: discountType === 'NONE' ? null : (entry.discountReason || '').trim() || null,
...
async function updatePackageRegistration(id: string, request: NextRequest) {
  ...
  const existing = await prisma.packageRegistration.findUnique({ where: { id } });
  ...
  if (
    ...
    const basePriceJod = clampNonNegative(Number(body.basePriceJod ?? existing.basePriceJod));
    const discountType = (body.discountType ?? existing.discountType ?? 'NONE').toUpperCase();
    const discountValue = discountType === 'NONE' ? null : Number(body.discountValue ?? existing.discountValue ?? 0);

    if (discountType !== 'NONE' && !(body.discountReason ?? existing.discountReason ?? '').trim()) {
    ...
    updateData.discountReason =
      discountType === 'NONE' ? null : (body.discountReason ?? existing.discountReason ?? '').trim() || null;
    updateData.finalPriceJod = computeFinalPriceJod(basePriceJod, discountType, discountValue);
    if (discountType !== 'NONE') {
  ...
  if (body.periodStartsAt !== undefined) updateData.periodStartsAt = body.periodStartsAt ? new Date(body.periodStartsAt) : null;
  if (body.periodEndsAt !== undefined) updateData.periodEndsAt = body.periodEndsAt ? new Date(body.periodEndsAt) : null;
...
async function markRegistrationPaid(id: string, request: NextRequest) {
  const body = (await request.json()) as { amountPaid: number; paymentMethod: string; privateNote: string; createdBy?: string };

  const registration = await prisma.packageRegistration.findUnique({ where: { id } });
  if (!registration) return jsonError('Registration not found', 404);
  if (!(body.privateNote || '').trim()) return jsonError('Private note is required');
...
async function markRegistrationUnpaid(id: string, request: NextRequest) {
  ...
  const voidReason = (body.voidReason || '').trim() || 'Marked as unpaid by staff';
...
async function voidReceipt(id: string, request: NextRequest) {
  ...
  if (!reason) return jsonError('voidReason is required when voiding a receipt');
  ...
  if (!receipt) return jsonError('Receipt not found', 404);
  if (receipt.voidedAt || receipt.status === 'VOIDED') return jsonError('Receipt is already voided');
...
async function dispatchGet(request: NextRequest, params: Params) {
  ...
  if (resource === 'package-pricing' && !id) {
    const rows = await prisma.packagePricing.findMany({ orderBy: { packageName: 'asc' } });
    return NextResponse.json(rows.map((row) => ({ packageName: row.packageName, basePriceJod: row.basePriceJod ?? null })));
  }
  ...
  if (resource === 'package-registrations' && action === 'session-adjustments') {
...
async function dispatchPost(request: NextRequest, params: Params) {
  ...
  if (resource === 'companies' && !id) {
    ...
    const company = await prisma.company.create({
      data: {
        ...
        contactEmail: String(body.contactEmail || '').trim() || 'infinitysportsacademyjo@gmail.com',
        ...
        status: (typeof body.status === 'string' ? body.status : 'ACTIVE') as any,
      },
    });
  ...
  if (resource === 'bookings' && !id) {
    ...
    const companyId = extractConnectId(body, 'company') || extractConnectId(body, 'companyId');
    ...
    if (!startTime || !endTime) return jsonError('Valid startTime and endTime are required');
    ...
    const data: any = {
      ...
      status: (typeof body.status === 'string' ? body.status : 'PENDING') as any,
    ...
    const classId = extractConnectId(body, 'class') || extractConnectId(body, 'classId');
    const coachId = extractConnectId(body, 'coach') || extractConnectId(body, 'coachId');
    const memberId = extractConnectId(body, 'member') || extractConnectId(body, 'memberId');
  ...
  if (resource === 'package-registrations' && !id) {
    ...
    try {
      ...
      return jsonError(error instanceof Error ? error.message : 'Failed to create registration');
    }
  }
...
async function dispatchPatch(request: NextRequest, params: Params) {
  ...
  if (resource === 'bookings' && !action) {
    ...
    if (body.facilityArea !== undefined) data.facilityArea = (body.facilityArea as string | null) ?? null;
    ...
    if (body.customerName !== undefined) data.customerName = (body.customerName as string | null) ?? null;
    if (body.customerPhone !== undefined) data.customerPhone = (body.customerPhone as string | null) ?? null;
    if (body.customerEmail !== undefined) data.customerEmail = (body.customerEmail as string | null) ?? null;
...
async function dispatchDelete(_request: NextRequest, params: Params) {
  ...
  if (resource === 'package-registrations') {
    try {
      ...
      if (error?.code === 'P2025') return jsonError('Registration not found', 404);
